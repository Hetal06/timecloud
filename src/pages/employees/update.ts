						...
						if (this.employeeCheckIns.length > 0) {
							...
							for (var iter = 0; iter < dataOfEmp.EmployeeData.length; iter++) {
								...
								if (iter == (dataOfEmp.EmployeeData.length - 1)) {
									...
									// this.employeeList = JSON.parse(localStorage.getItem("employeeList"));
									this.EmpStoreData = JSON.parse(localStorage.getItem("empStatusUpdate"));
									console.log("line 180", this.EmpStoreData.employeeNo);
									// console.log("line 181", this.employeeList);
									for (var itr = 0; itr < this.employeeList.length;itr++){
										// console.log("employee line 183", this.employeeList[itr]);
										if (this.employeeList[itr].employeeNo == this.EmpStoreData.employeeNo){
											console.log("line 185", this.EmpStoreData.employeeNo, "===", this.employeeList[itr].employeeNo);
											// console.log("checktype line 186", this.EmpStoreData.checkType, "!===", this.employeeList[itr].status)
											this.employeeList[itr].status = this.EmpStoreData.checkType;
										}
									}
								}
							}
